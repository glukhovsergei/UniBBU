# Stability analysis for dipole modes (approximated in case of multiple recirculations)
...
NumberOfModeEnsembles = NmDipole // 2 if ActiveModes[0] < 0 else 1
...
for MachineIndex in range(InitialMachineIndex, FinalMachineIndex):
    ...
    for ModeEnsembleIndex in range(NumberOfModeEnsembles):
        ...
        for i in range(Nc):
            ...
            C[i] = [math.cos(j) for j in Theta[i]]
            S[i] = [math.sin(j) for j in Theta[i]]
        if len(ActiveModes) == 0:
            print('ERROR: No active dipole modes.')
            sys.exit(0)
        stream.close()
        print("modes: ", end = "")
        print(*ActiveModes)
        ...
        T12 = [[0.] * Nc for i in range(Nc)]
        T34 = [[0.] * Nc for i in range(Nc)]
        T32 = [[0.] * Nc for i in range(Nc)]
        T14 = [[0.] * Nc for i in range(Nc)]
        for i in range(Nc):
            for j in range(Nc):
                ...
                for n in range(4):
                    stream.readline()
                T12[i][j] = list(map(float, stream.readline().split()))
                T14[i][j] = list(map(float, stream.readline().split()))
                T32[i][j] = list(map(float, stream.readline().split()))
                T34[i][j] = list(map(float, stream.readline().split()))
                stream.readline()
        stream.close()
        ...
        weff = (1-dww)*2*math.pi*(f[0][0]-f0)
        weff_final = (1+dww)*2*math.pi*(f[0][0]-f0)
        flag = 0
        Icur = [0.] * NcNm
        sortedIcur = [0.] * NcNm
        Iprev = [0.] * NcNm
                    ...
                    for j in range(Nc):
                        for jm in range(Nm):
                            ...
                            for k in range(len(p0[i][j])):
                                D = T12[i][j][k]*ci*cj + T32[i][j][k]*si*cj + T14[i][j][k]*ci*sj + T34[i][j][k]*si*sj
